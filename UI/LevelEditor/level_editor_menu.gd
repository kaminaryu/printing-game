...
func _generate_grid_preview() -> void :
	var path: String = "res://Resources/Levels/%d.tres" % selected_level
	
	if ResourceLoader.exists(path):
		var level_data = load(path) as LevelData
		preview_grid.update_preview(level_data)
		return
